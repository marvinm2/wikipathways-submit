...
1. **What exactly is the placeholder?** `pathways/WP1/` is a real pathway in the tree, so "WP1"
   cannot double as the placeholder path. The sandbox has a `WP0001` branch and a closed PR
   "TEST Create WP0001.gpml", which suggests `WP0001` is the convention there. Confirm with
   Marvin, and confirm whether the placeholder appears in the *path*, the *Version* attribute,
   or both.
2. **Branch name format.** `<username>/WP0001` collides the second time the same person submits.
   Options: a counter, a slug of the pathway title, or a timestamp — the PathVisio plugin already
   uses `contribution-<epoch-ms>` in the sandbox.
3. **Who does the rename at approval** — the app (commit on the branch, then merge) or a workflow
   in the content repo (after merge)? The sandbox already has `3a_approved_pull_request.yml`,
   dispatched by an `accepted` **label** (`pr_label_dispatcher.yml`), which is a natural seam: the
   app could apply the label and let the repo's own pipeline finish the job.
4. **How does this app relate to the sandbox's existing pipeline?** The sandbox is not a bare
...
5. **Do we still need the lock for updates?** Yes — that is about concurrent edits to an existing
...
`app/submit/service.py` and `app/wpid/allocator.py`. Settle question 1 and 2 with Marvin before
touching code — the placeholder and the branch name determine everything else.
